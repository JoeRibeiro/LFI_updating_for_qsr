INDPLOTFN <- function(DATA2PLOT=DATA2PLOT, BOOTDATA2PLOT=BOOTDATA2PLOT,YRS=YRS,YLIM=NULL,YLAB=YLAB,TITLE=TITLE,GAMMOD=GAMMOD,
                      ...
                      ADDLOESS=T,ADDLAST6LM=T,FILENAM3=FILENAM2,SP='ALL_species'){
  #BOOTSTRAP<-T; ADDBOOTTREND<-T; ADDBOOTTREND_CI<-F; ADDBOOT_ERRBAR<-T; ADDGAM<-T; BEST_AND_BOOT<-T
  #BOOTSTRAP<-F; ADDBOOTTREND<-F; ADDBOOTTREND_CI<-F; ADDBOOT_ERRBAR<-F; ADDGAM<-T; BEST_AND_BOOT<-T
  #DATA2PLOT<-DATA2PLOT[,n]; YRS<-an(names(DATA2PLOT))
  plot(YRS,DATA2PLOT,col='white',ylim=YLIM,ylab=YLAB)
  title(TITLE)
  if( (ADDGAM & (!ADDBOOTTREND_CI | !BOOTSTRAP)) | BEST_AND_BOOT)  points(YRS,   DATA2PLOT,col=3, pch=4); 
  ...
  if(ADDLOESS){
    ...
    polygon( c(YRS, YRS[length(YRS):1]), c(PRED$fit+PRED$se.fit, (PRED$fit-PRED$se.fit)[length(YRS):1] ),col='light blue')
    ...
    JR_INDout <- data.frame(Year=YRS,Index_val=DATA2PLOT,Min_Line=min(DATA2PLOT[-((length(DATA2PLOT)-5):length(DATA2PLOT))],na.rm=T), 
                         Upper_bound=PRED$fit+PRED$se.fit, Lower_bound=PRED$fit-PRED$se.fit, loessline=PRED$fit)
    print(paste(FILENAM3,SP,'region',str_replace_all(TITLE, "[^[:alnum:]]", " "),'_plotvals.csv',sep="_"))
    write.csv(JR_INDout,paste(FILENAM3,SP,'region',str_replace_all(TITLE, "[^[:alnum:]]", " "),'_plotvals.csv',sep="_"),row.names=F)
    #print(JR_INDout)
